...
\begin{abstract}
  ...
  To evaluate these collaborative abilities, we introduce \textsc{LLAMIA-Bench}, a suite of six collaborative chess tasks from prominent AI literature spanning: behavior cloning, state understanding, and natural-language explanation of actions and policies. These tasks cannot be solved by LLMs or subagents independently, making them a perfect testbed for studying collaboration. Our experiments reveal a consistent \emph{debt} introduce:
  the performance gap between verbalized and internalized integration  widens throughout training \yaman{why did we suddenly bring up training?} and persists as the LLM scales from 4B to 14B parameters. To close this gap, we introduce \emph{latent state internalization}, which
