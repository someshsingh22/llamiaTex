...
\begin{table*}[!h]
    ...
    LLAMIA-14B [Ours] & SALT & & 30M & 14B & \valgood{2279~($\pm45$)} & 92.9 \\
    \midrule[1pt]
...
\begin{table*}[!h]
    ...
    LLAMIA-14B & 14B & SALT & \valbest{0.61} & \valbest{0.91} & \valbest{5.09} & \valbest{57.03} \\
    \bottomrule[1.2pt]
    \end{tabular}
    \end{adjustbox}
    \caption{
    ...
    LLAMIA-14B significantly outperforms all baselines, achieving 0.61 G-eval and 57.03 BLEU, while reducing highlight RMSE to 5.09 suggesting it generates not only fluent but strategically informative and attention-worthy narratives. 
    }
...
Players also adapt their strategies based on their opponents’ skill levels, especially when there is a significant skill gap. Such matchups typically occur in tournaments or exhibitions. To capture playing styles in these conditions, we filter games where players had an Elo difference greater than 500. This yields only 34k games in total.
    ...
    \textbf{ChessGPT-7B + Lc0\textsuperscript{\textdagger}} & IFT, 2-shot, Tool & 1 & 10M & 0.15 & 0.13 & 0.10 & 0.13 & 0.14 & 0.13 & 0.13 & 0.18\\
        ...
        \multirow{5}{*}{\textbf{ LLAMIA-14B}} & PT, 2-shot & 1 & - & 0.16 & 0.18 & 0.21 & 0.20 & 0.29 & 0.43 & 0.39 & 0.36\\
...
\begin{table*}[!h]
    ...
    LLAMIA-14B & 14B & SALT, 0-shot & \valbest{32.05} & \valbest{67.11} & \valbest{38.13} & \valbest{45.12} & \valbest{49.13} & \valbest{5.66} \\
    \bottomrule[1.2pt]
    \end{tabular}
    \end{adjustbox}
...
Each model plays 50 games per motif category (250 total across 5 categories) with randomized openings from the ECO database. All Elo estimates include 95\% bootstrap confidence intervals over 1,000 resamples.
...
\end{itemize}
LLAMIA-14B achieves 11\% average win-rate improvement on OOD instructions (vs. 17\% on in-distribution), indicating partial but meaningful generalization to novel strategic directives.
