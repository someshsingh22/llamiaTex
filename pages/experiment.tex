...
\begin{figure*}[t]
    ...
    \label{fig:combined}
\end{figure*}
LLAMIA-14B leads all six tasks with a single multi-task model (\cref{tab:main_bench}; \cref{fig:combined}c), outperforming GPT-5\,+\,Lc0 by $+8$ to $+42$ points across the suite.
On behavior cloning, LLAMIA-14B lands within $2$\,pp of Allie-Adaptive-Search on the in-distribution Maia split ($53$ vs.\ $55$) and surpasses it by $+12$\,pp on Wild ($57$ vs.\ $45$), where blitz-only training data misses grandmaster play, time pressure, and large rating gaps.
...
Puzzle Interest requires ranking positions by community-derived interestingness, a signal that depends on the engine's policy distribution and value gradients across candidate moves (\cref{sec:app_landscape_interest}).
...
The margin here is qualitative, not incremental: verbalization has zero useful signal for this task, while latent tokens give the LLM direct access to the distributional structure that defines interestingness.
The variation in this gap across tasks, from $+8$ on in-distribution BC to $+44$ on Interest (\cref{tab:main_bench}, $\Delta$ row), is the subject of the next section.
...
\somesh{Now that we have quantitatively differentiated LLAMIA on all tasks against Frontier + Task specific Finetunes we discuss the verbalization debt through key headings, first, define it. Summarize the results from table 1, then from figure Verbalization gap is consistent across scale, and throughout training, from training dynamics figure.}

LLAMIA and LLAMIA-Verb share the same 14B backbone, Lc0-BT4 subagent, and DAPO recipe; the only difference is whether the subagent's state reaches the LLM as latent tokens or as verbalized text. We define the resulting performance gap as the \textbf{verbalization debt}, reported per task in the $\Delta$ row of \cref{tab:main_bench}.

\paragraph{The debt is task-specific and tracks what verbalization discards.}
The margin spans an order of magnitude: $+44$ on Interest, $+35$ on Commentary, $+22$ on Difficulty, $+18$ on Wild BC, $+17.1$ on Annotation, and $+8$ on MAIA BC. At one extreme, the policy shape and value gradients that distinguish engaging puzzles from routine ones have no text equivalent; every verbalized system scores ${\leq}12$ on Interest regardless of model scale (\cref{fig:combined}c), while LLAMIA reaches $52$. At the other, the engine's top-$k$ moves already cover most human play, leaving a modest residual on in-distribution behavior cloning. \cref{sec:app_landscape} traces each task's debt to the specific features that verbalization discards.

\paragraph{The gap persists across backbone scale.}
Increasing the LLM from 4B to 14B improves both systems, but the debt persists at every scale (\cref{tab:full_bench}). On Interest, LLAMIA-Verb-14B scores $8$ while LLAMIA-4B already reaches $38$. The bottleneck is the interface, not model capacity.

\paragraph{The gap widens throughout training.}
