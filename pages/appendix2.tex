...
The vLLM/PyTorch/Transformers versions are tightly coupled and cannot be changed independently.
vLLM~0.10.2 is the highest release inside VERL~0.7.1's supported range (${\geq}\,0.8.5$, ${\leq}\,0.12.0$) that ships the \texttt{qwen3\_next.py} model file.
Its pre-built CUDA wheel calls the two-argument form of \texttt{c10::cuda::SetDevice}, present only in PyTorch~${\geq}\,2.8$.
Transformers~${\geq}\,5$ removes \texttt{all\_special\_tokens\_extended} from the tokenizer base class, which vLLM~0.10.2's tokenizer wrapper still references; we pin to 4.56.2.
...
A special token $\langle\text{state}\rangle$ anchors each injection site; $k$ contiguous positions immediately following it are overwritten with $\vz_t = H_\varphi(\vh_{s_t})$ before the LLM forward pass, following the multimodal alignment approach of LLaVA~\cite{liu2023visual}.
...
\bottomrule
\end{tabular}
\caption{Training hyperparameters. Entries marked (--) are to be filled upon final experimental runs.}
\label{tab:hparams}
\end{table}
...
The ablation in \S\ref{sec:stage_ablation} confirms that skipping Stage 1 causes training instability during the first 40\% of Stage 2.
...
\paragraph{Throughput / batch profile.}
Resident GPU memory is flat across batch sizes (Table~\ref{tab:lc0-bench}): the cuBLAS workspace plus fp16 weights dominate, and per-position activations are negligible.
\texttt{MinibatchSize=128} gives 3$\times$ the throughput of \texttt{MinibatchSize=8} at no memory cost.
...
\bottomrule
\end{tabular}
\caption{\texttt{lc0 backendbench} sweep on A100-80GB, BT4 + cuda-fp16. Memory is constant at ${\sim}\,3$\,GiB regardless of batch size; throughput plateaus near batch 128--256. Co-residing engines on the same A100 need only ${\sim}\,4$\,GiB (${\sim}\,5\%$) headroom per GPU.}
\label{tab:lc0-bench}
\end{table}

\paragraph{Wrapper configuration.}
Environment variables with defaults: \texttt{LC0\_BIN} (engine path), \texttt{LC0\_NET} (\texttt{.pb.gz} weights), \texttt{LC0\_BACKEND} (\texttt{cuda-fp16}), \texttt{LC0\_THREADS} (\texttt{2}), \texttt{LC0\_MINIBATCH} (\texttt{128}), \texttt{LC0\_MAX\_PREFETCH} (\texttt{32}), \texttt{LC0\_NNCACHE} (\texttt{200000}), \texttt{LC0\_PORT} (\texttt{7100}).
The multi-GPU deployment script \texttt{start\_lc0\_servers.sh} launches eight instances in parallel, assigning \texttt{CUDA\_VISIBLE\_DEVICES=}$i$ and \texttt{LC0\_PORT=}$7100{+}i$ for $i\in\{0,\ldots,7\}$, then health-checks all eight before returning.
HTTP-level concurrency within each process is serialised by an internal lock because UCI is a single-conversation protocol.
Both \texttt{/analyze} and \texttt{/policy} accept an optional \texttt{moves} field (UCI-string list) applied on top of the FEN, so BT4's seven history planes are filled with the real game rather than synthesised; this matters for raw-policy queries mid-game.
...
The HTTP interface described above is sufficient for all verbalized-access baselines but does not expose hidden activations.
The latent internalization path (\S\ref{sec:methodology}) requires a separate forward-pass shim that loads the same BT4 \texttt{.pb.gz} weights into PyTorch and emits the post-attention residual stream at layer 15, producing $\vh_s \in \R^{1024}$.
This vector is then projected by $H_\varphi$ into $k{=}16$ token embeddings injected into the LLM trace.
The shim runs out of process and is not part of \texttt{lc0\_server}.
...
The prompt is compact (${\approx}\,220$ tokens).
Three clauses encode the harness's invariants: (i) the ``do not call \texttt{reset\_position}'' clause prevents the agent spending a round verifying a FEN load the harness already performed; (ii) the \texttt{moves=[\ldots]} clause teaches the no-mutate analysis primitive, collapsing the standard \texttt{make\_move}$\to$\texttt{analyze}$\to$\texttt{undo\_move} sequence into one call; (iii) the efficiency clause encourages batching of independent calls.
...
\begin{tcolorbox}[warnbox, title={$\mathbf{!}$\; Tool-Call Format --- Qwen3 XML (not Hermes JSON)}]

\textbf{Root cause.}
Qwen3's chat template generates tool calls in XML-like format:
\texttt{<tool\_call><function=\textit{name}><parameter=\textit{key}>\textit{value}</parameter></function></tool\_call>}.
vLLM's \texttt{--tool-call-parser hermes} locates the \texttt{<tool\_call>} tags correctly but expects JSON inside; failing to parse the XML body, it leaves \texttt{msg.tool\_calls = []} and returns the raw XML in \texttt{msg.content}.

\medskip
\textbf{Fix.}
\texttt{\_parse\_qwen\_tool\_calls(content)} in \texttt{agents/chess\_analyst.py} uses two regex patterns---\texttt{QWEN\_TOOL\_RE} (matches the outer function block) and \texttt{PARAM\_RE} (extracts and \texttt{json.loads}-coerces parameter values)---to reconstruct the tool call.
Tool results are returned as standard \texttt{role=tool} messages; vLLM reformats them correctly into Qwen3's tool-response template server-side.
Any code that reads \texttt{msg.tool\_calls} directly from the OpenAI client will silently drop all tool calls in this configuration; always use \texttt{\_parse\_qwen\_tool\_calls} or its equivalent.
\end{tcolorbox}
