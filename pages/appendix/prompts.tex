...
Dynamic re-encoding (\texttt{<invoke>}) fires after every board-mutating tool call
...
\smallskip
\noindent\textbf{Note:} \texttt{get\_policy} is the \texttt{<invoke>} call.
Invoking it causes the harness to run a fresh BT4 forward pass on the current position
and inject $k{=}32$ latent state tokens (\texttt{<state>}) into the next LLM turn.
\end{tcolorbox}
