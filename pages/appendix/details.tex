...
Three prompt regimes govern the pipeline: Stage~1 projector alignment, Stage~2 DAPO rollouts, and the shared dynamic re-encoding (\texttt{<invoke>}) format. Extended per-task LLAMIA-Bench listings appear in Appendix~\ref{sec:app_prompts}.
...
All six task families (behaviour cloning, stylometry, puzzle understanding,
move annotation, game commentary, instruction-conditioned play) share the same
tool catalogue as the toy task.
...
Dynamic re-encoding (\texttt{<invoke>}) fires after every board-mutating tool
