...
\caption{%
  ...
  \somesh{1. Remove gameplay and Stylom. 2. Avoid acronyms. 3. say in the caption we used the same test split as authors if given otherwise a random split. 4. Instead of a separate column ref after the task/split. 5. Merge In the wild task splits in MAIA calling it just In the wild, dataset is MAIA-KDD filtered and source is Lichess (\textbf{Ours}). 6. Group common cells like Lichess Puzzles Task expert etc and add their link cites etc in sources, combine dataset and source columns it should have source of dataset, citation, or Ours. 7. Agadmator was made by us. there is no IND in ICP remove that}
