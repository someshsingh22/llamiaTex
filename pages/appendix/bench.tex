...
Puzzle Understanding & Lichess Puzzles \cite{lichess2024puzzles} & 4M / \tbd & \cite{milosz2024predicting} \\
Move Annotation & Lichess \cite{jhamtani-etal-2018-learning} & 90K / authors' & SCC \cite{zang2019automated} \\
Game Commentary & Agadmator YouTube (\textbf{(Ours)}) & 1.9K / 100 games & --- \\
Instr.-Cond.\ Planning & Synthetic Motifs\textbf{(Ours)} & 250/motif / 250 & --- \\
\bottomrule
\multicolumn{4}{l}{\textsuperscript{*}\,Plus three OOD splits (GM-25, Low-Time, Elo Gap); see \S\ref{sec:app_bench_bc}.}
\end{tabular}
\end{adjustbox}
\end{table}
...
Puzzle Understanding  & Spearman $\rho$ $\uparrow$                & Normalized MAE $\downarrow$ \\
Move Annotation       & G-eval $\uparrow$;\enspace BLEU-2 $\uparrow$ & G-eval \\
Game Commentary       & G-eval $\uparrow$;\enspace BLEU-2 $\uparrow$ & G-eval \\
Instr.-Cond.\ Planning\textsuperscript{\textdagger} & Tournament Elo $\uparrow$ & --- \\
\bottomrule
\multicolumn{3}{l}{\textsuperscript{\textdagger}\,Test-only task; no RL reward signal.}
\end{tabular}
\end{adjustbox}
\end{table}
