...
\caption{\textbf{LLAMIA-Bench: dataset provenance.} \textbf{Ours} = constructed for this work. OOD = out-of-distribution test split (\S\ref{sec:app_bench_bc}).}
...
\midrule
\makecell[tl]{Behavior Cloning \\ {\scriptsize MAIA-KDD, Lichess~\cite{mcilroy2020aligning}}}
...
\makecell[tl]{\quad\emph{In the wild} (OOD) \\ {\scriptsize MAIA-KDD, Lichess (\textbf{Ours})}}
  ...
  & $\leq$167K / --- \\
\midrule
\makecell[tl]{Puzzle Understanding \\ {\scriptsize Lichess Puzzles~\cite{lichess2024puzzles}}}
  & Predict difficulty (Glicko-2) and interest (community votes) of tactical puzzles
  & 4M / 5K \\
\midrule
\makecell[tl]{Move Annotation \\ {\scriptsize Lichess~\cite{jhamtani-etal-2018-learning}}}
  & Generate natural-language explanation for a single move across 5 semantic categories
  & 90K / authors' \\
\midrule
\makecell[tl]{Game Commentary \\ {\scriptsize Agadmator YouTube (\textbf{Ours})}}
  & Produce coherent multi-turn narrative spanning an entire game
  & 1.9K / 100 \\
\midrule
\makecell[tl]{Instr.-Cond.\ Planning \\ {\scriptsize Synthetic Motifs (\textbf{Ours})}}
